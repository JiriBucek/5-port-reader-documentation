...
- Grouped test history may optionally be restored from backend after reset or after a later login if BioEasy judges that useful, but only records created on this device should be shown; do not restore grouped test records from other devices that used the same operator credentials.
